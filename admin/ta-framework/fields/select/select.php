 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'TAF_Field_select' ) ) {
	class TAF_Field_select extends TAF_Fields {
			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
					...
					if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
						...
						echo '<select name="' . $field_name . '" class="taf-hide-select hidden"' . $multiple_attr . $field_attr . '>';
