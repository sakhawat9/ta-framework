 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'TAF_Field_textarea' ) ) {
	class TAF_Field_textarea extends TAF_Fields {
		...
		public function render() {
			...
			echo '<textarea name="' . esc_attr( $this->field_name() ) . '"' . wp_kses_post( $this->field_attributes() ) . '>' . $this->value . '</textarea>';
		...
		public function shortcoder() {
			...
			if ( ! empty( $this->field['shortcoder'] ) ) {
				...
				$instances  = ( ! empty( TAF::$shortcode_instances ) ) ? TAF::$shortcode_instances : array();
